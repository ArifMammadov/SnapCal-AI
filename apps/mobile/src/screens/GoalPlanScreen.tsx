...
import { t, formatNumberLocalized } from '../lib/i18n.js'
  ...
  return (
    <div style={{ position: 'relative', width: size, height: size }}>
      ...
      >
        ...
        <span style={{ fontSize: 10, color: 'var(--text-secondary)' }}>{t('completed')}</span>
      </div>
    </div>
  )
}
...
export function GoalPlanScreen({ onClose }: GoalPlanScreenProps) {
  ...
  if (loading) {
    return (
      <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)' }}>
        <p style={{ color: 'var(--text-secondary)' }}>{t('loadingPlanDots')}</p>
      </div>
    )
  }
  ...
  if (error || !plan) {
    return (
      <div style={{ height: '100%', padding: 56, background: 'var(--bg)' }}>
        <button onClick={onClose} className="back-button" style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', color: 'var(--green)', fontSize: 15, cursor: 'pointer', fontFamily: 'inherit', padding: 0, marginBottom: 16 }}>
          <BackIcon size={18} />
          {t('back')}
        </button>
        <p style={{ color: 'var(--rose)' }}>{error || t('failedLoadPlan')}</p>
      </div>
    )
  }
  ...
  return (
    <div className="no-scrollbar" style={{ height: '100%', overflowY: 'auto', background: 'var(--bg)' }}>
      <div style={{ padding: '56px 20px 20px', background: 'linear-gradient(180deg, var(--bg-card) 0%, var(--bg) 100%)', borderBottom: '1px solid var(--border)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
          ...
          <div>
            <h1 className="font-display" style={{ fontSize: 22, fontWeight: 700, color: 'var(--text-primary)', margin: 0, letterSpacing: '-0.5px' }}>
              {t('transformationPlan')}
            </h1>
            <p style={{ fontSize: 13, color: 'var(--text-secondary)', margin: 0 }}>{t('personal6MonthPlan')}</p>
          </div>
        </div>
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-secondary)', margin: '0 0 10px' }}>
              {plan.startWeightKg} → {plan.targetWeightKg} {t('kg')}
            </p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
              {[
                { label: t('startWeight'), value: `${plan.startWeightKg} ${t('kg')}` },
                { label: t('targetWeightPlan'), value: `${plan.targetWeightKg} ${t('kg')}` },
                { label: t('totalLoss'), value: plan.totalLossKg ? `${plan.totalLossKg} ${t('kg')}` : '—' },
              ].map((s) => (
        ...
        >
          {generating ? t('generatingPlan') : t('generateAiPlan')}
        </button>
      </div>
      ...
      <div style={{ padding: '20px 20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 24 }}>
          <TargetPill label={t('calories')} value={formatNumberLocalized(daily.calories ?? 0)} unit="kcal" color="#22c55e" />
          ...
          <TargetPill label={t('workouts')} value={daily.workoutsPerWeek || '—'} unit={t('daysPerWeek')} color="#8b5cf6" />
        </div>
                ...
                >
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <span style={{ fontSize: 11, color: m.color, fontWeight: 700, letterSpacing: '0.04em' }}>{t('month')} {m.month}</span>
                        {m.current && <span style={{ padding: '2px 8px', background: m.color, borderRadius: 6, fontSize: 10, color: '#fff', fontWeight: 700 }}>{t('current')}</span>}
                      </div>
                      <span className="font-display" style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-primary)' }}>
                        {m.targetWeightKg ? `${m.targetWeightKg} ${t('kg')}` : '—'}
                      </span>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      {[
                        ...
                        { label: t('workouts'), value: `${m.workoutsPerWeek} ${t('daysPerWeek')}` },
                      ].map((d) => (
                  ...
                  {expandedMonth === m.month && m.weeks && m.weeks.length > 0 && (
                    <div style={{ marginTop: 14, paddingTop: 14, borderTop: '1px solid var(--border)' }}>
                      <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-secondary)', margin: '0 0 10px' }}>{t('weeklyChecklist')}</p>
                          ...
                          <div key={w.week} style={{ padding: 10, background: 'var(--bg-elevated)', borderRadius: 10 }}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                              ...
                              <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>
                                {w.calorieTarget} kcal · {w.workoutDays} {t('daysPerWeek')}
                              </span>
                            </div>
